...
public class Arbol_Rep_Grafica extends JPanel {
     ...
     * inicializar los atributos de la clase y llama al método repaint(), que es
     * el encargado de pintar el Arbol.
     *
     * @param miExpresion: dato de tipo ArbolExpresion que contiene el Arbol a
     * dibujar.
     */
    public Arbol_Rep_Grafica(ArbolBB miArbol) {
    ...
    private void calcularPosiciones() {
        ...
        if (root != null) {
            calcularTamañoSubarbol(root); // llamada a metodo
    ...
    private Dimension calcularTamañoSubarbol(Nodo n) {
        ...
        Dimension ld = calcularTamañoSubarbol(n.getIzq()); //Nodo de referencia para calcular el tamaño
        Dimension rd = calcularTamañoSubarbol(n.getDer());

        int h = fm.getHeight() + parent2child + Math.max(ld.height, rd.height); // altura
        int w = ld.width + child2child + rd.width; // ancho

        Dimension d = new Dimension(w, h);  //Contiene la altura y el ancho del componente
    ...
    private void calcularPosicion(Nodo n, int left, int right, int top) {
        ...
        Dimension ld = (Dimension) subtreeSizes.get(n.getIzq()); //obtencion del dato izquierdo
        if (ld == null) {
            ld = empty;
        }

        Dimension rd = (Dimension) subtreeSizes.get(n.getDer()); // obtencion del dato derecho
        if (rd == null) {
            rd = empty;
        }
        ...
        int width = fm.stringWidth(n.getDato() + ""); // ancho que cubre el tamaño del dato

        posicionNodos.put(n, new Rectangle(center - width / 2 - 3, top, width + 6, fm.getHeight())); // Tamaño del rectangulo
    ...
    private void dibujarArbol(Graphics2D g, Nodo n, int puntox, int puntoy, int yoffs) {
        ...
        Rectangle r = (Rectangle) posicionNodos.get(n); // La variable g permite realizar el dibujo 
        g.draw(r);                                      // de las lineas, rectangulos y del string
        g.drawString(n.getDato() + "", r.x + 3, r.y + yoffs); // yoffs es la altura  del fontMetrics

        if (puntox != Integer.MAX_VALUE) {
            g.drawLine(puntox, puntoy, (int) (r.x + r.width / 2), r.y);
        }

        dibujarArbol(g, n.getIzq(), (int) (r.x + r.width / 2), r.y + r.height, yoffs); // cada nodo de la izquierda
        dibujarArbol(g, n.getDer(), (int) (r.x + r.width / 2), r.y + r.height, yoffs); // cada nodo de la derecha
